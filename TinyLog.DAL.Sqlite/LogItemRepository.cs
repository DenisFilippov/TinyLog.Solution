...
public class LogItemRepository : BaseRepository, ILogItemInserterRepository, ILogItemClearRepository
{
  ...
  private async Task InsertSingleAsync(Item item, SqliteConnection connection, SqliteCommand command, CancellationToken token)
  {
    ...
    if (item.Tags.Any())
    {
      await _logTagInserterRepository.InsertAsync(item, connection, token);
    }
  }
