...
internal class LogTagRepository : BaseRepository, ILogTagInserterRepository
{
  ...
  public async Task InsertAsync(Item item, SqliteConnection connection, CancellationToken token)
  {
    ...
    foreach (var tag in item.Tags) await InsertSingleAsync(item.Id, tag.Key, tag.Value, command, token);
  }
  ...
  private static async Task InsertSingleAsync(long logItemsId, string key, byte[]? value, SqliteCommand command,
